...
global apkSelfApplication                           # apk自身的Application
            ...
            if not os.path.exists(targetFile) or (os.path.exists(targetFile) and (os.path.getsize(targetFile) != os.path.getsize(sourceFile))):
                if '.smali' in file and fileN in fileNameSet.keys():
                    ...
                    while True:
                        ...
                        if App in file:
                            if len(apkSelfApplication)>0 and 'Landroid/app/Application' in line:
                                line = line.replace('Landroid/app/Application', 'L' + apkSelfApplication)
                                print(line)
...
# 处理Application的类，修改super的调用者为apk原有的Application类
        ...
        if '<application' in line:
            ...
            if 'android:name=' in line:  # 如果有入口获取其入口类的全路径
                ...
                if result:
                    ...
                    # 替换为插包代码的Application
                    p = re.compile('android:name="[^"]*"')
                    line = p.sub('android:name="'+applicationName+'"', line)

                    apkSelfApplication = className.replace(".", "/")
        ...
        for fileName in files:
            ...
            if '.apk' in srcFilePath and '.DS.Store' not in srcFilePath:
                ...
                try:
                    ...
                    # 如果有Application入口，修改为插包代码的Application
                    handleManifest()
                    # 复制插包smali代码，修改插包代码的Application让其继承Apk自身的Application类
